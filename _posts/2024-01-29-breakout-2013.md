...
```
**Fig 1:**  Experience Replay implementation
...
**Fig 3:**  Processed input to the network
...
class DQNDeepmind2013(nn.Module):
    ...
    def forward(self, s):
        return self.conv(s) 
```
**Fig 3:**  Pytorch implementation of the 2013 conv net
...
```python
    def select_next_action(self, s, epsilon):
        with torch.no_grad():
            ...
            return a
```
**Fig 4:**  DQN action selection logic
...
The average validation reward for our model on the last epoch was 16 compared to the published score of 168. Assuming these are unclipped scores across an entire episode of 5 lives, it provides a benchmark for comparison. I suspect that had we increased the capacity of the experience replay then we would have seen a higher score. However, I'm not sure if it would have been enough to reach the published score.
...
Next time we'll look at the 2015 Nature paper, the innovations that were introduced and see if we can improve our score further.
